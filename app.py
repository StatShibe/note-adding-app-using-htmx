...
def index():
    ...
    if not token:
        ...
        return render_template("index.html", is_htmx=is_htmx, notes=notes, user = data['user'])
...
def login_page():
    if request.method == 'POST':
        ...
        if user_details is None:
            ...
            username == user_details['username'] and
...
def add_note():
    if request.method == 'GET':
        ...
        try:
            ...
            conn.execute(
                "INSERT INTO notes (owner_username, content, posted_at_time) VALUES (?, ?, ?)",
                (username, request.form['note'], datetime.datetime.now())
            )
            conn.commit()
            conn.close()
        except Exception as e:
...
def update_note():
    ...
    if request.method == 'GET':
        ...
        conn.execute(
            "UPDATE notes SET content = (?), posted_at_time = (?) WHERE note_id = (?)",
            (new_content, datetime.datetime.now(), note_id)
        )
        conn.commit()
        conn.close()
